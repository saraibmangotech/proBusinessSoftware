...
function CustomUnDueStatement() {
    ...
    const tableHead = ['Customer ID', 'Customer Name', 'VIN', 'LOT', 'Make', 'Modal', 'Color', 'Status', 'Custom Duty']
    ...
    const getVatDue = async (page, limit, filter) => {
        ...
        try {
            ...
            const { data } = await VccServices.getCustomUnDueStatement(params)
            setVehicleList(data?.dues)
            setStatementData(data)
            setOriginalVehicle(data?.dues)
            const totalValue = data?.dues.reduce((acc, entry) => acc + parseFloat(entry.vcc?.custom_charges_aed ? entry.vcc?.custom_charges_aed : 0), 0);
    ...
    const renderCellContent = (colIndex, item, isActive,) => {
        ...
        switch (colIndex) {
            ...
            case 8:
                return parseFloat(item?.vcc?.custom_charges_aed ? item?.vcc?.custom_charges_aed : 0).toFixed(2) ?? '-';
            default:
                return "-";
        }
    };
            ...
            <Box className={"blur-wrapper"} sx={{ height: '100vh', backgroundColor: '#fff', width: '100%', zIndex: '1111' }} >
                ...
                >
                    ...
                    <Box sx={{ m: 4, my: 1, }}>
                        ...
                        >
                            <Typography variant="h5" sx={{ color: Colors.charcoalGrey, fontFamily: FontFamily.NunitoRegular, fontSize: '20px' }}>
                                Custom UnDue Report
                            </Typography>
...
export default CustomUnDueStatement;