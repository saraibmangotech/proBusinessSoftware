...
import { Box, CircularProgress, Container, FormControl, FormControlLabel, Grid, Paper, Radio, RadioGroup, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography, tableCellClasses } from "@mui/material";
...
function VaultTopUp() {
  ...
  // *For Get Vault Top Up Preview
  const getVaultTopUpPreview = async () => {
    setLoader(true)
    try {
      let params = {
        topup_id: atob(id).split('-')[1]
      }
      const { data } = await FinanceServices.getVaultTopUpPreview(params)
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: 'wallet-topup',
  });

  useEffect(() => {
    if (id) {
      getVaultTopUpPreview()
    }
  }, [id]);
            ...
            <Box sx={{ p: 5 }} >
              <Grid container spacing={2} alignItems='flex-start'>
                ...
                <Grid container spacing={2} item xs={12} sm={6}>
                  ...
                  <Grid item md={6}>
                    <Box sx={{ mt: 3 }}>
                      <QRCodeCanvas value={window.location.origin + `/vaulttopup-preview/${id}`} />
                    </Box>
                  </Grid>
                </Grid>
                <Grid container spacing={2} item xs={12} sm={6}>
                  ...
                  <Grid item md={6}>
                    <Typography noWrap variant="body1" className={classes.text}>
                      {vaultTopUpDetail?.receipt_voucher}
                    </Typography>
                  </Grid>
                  <Grid item md={12}>
                    <Box sx={{ p: 1 }}>
