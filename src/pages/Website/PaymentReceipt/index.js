...
function PaymentReceiptPreview() {
  ...
  // *For Get Payment Receipt Preview
  const getPaymentPreview = async () => {
    ...
    try {
      ...
      const { data } = await VehiclePaymentServices.getPaymentPreview(params)
      setPaymentReceiptDetail(data?.details)
    } catch (error) {
      ErrorToaster(error)
    } finally {
      setLoader(false)
  ...
  return (
    <Container>
      ...
      <Box sx={{ width: '1000px', mx: 4, my: 2, bgcolor: Colors.white, boxShadow: '0px 8px 18px 0px #9B9B9B1A' }}>
        ...
        {!loader ? (
          <PDFExport ref={contentRef} fileName="Receipt">
            <Box >
              <Grid container spacing={0}>
                <Grid item md={3.5}>
                  <Box component={'img'} src={Images.usedCarLogo} sx={{ height: '90px', my: 1.5, mx: 4 }} />
                </Grid>
                <Grid item md={8.5}>
                ...
                <Grid item md={5.5}>
                  <Box sx={{ ml: 4 }}>
                    <Grid container spacing={0.5} >
                      <Grid item md={12}>
                        <Typography variant="h4" className={classes.text} sx={{ mb: 1 }}>Receipt To:</Typography>
                      </Grid>
                      <Grid item md={5}>
                      ...
                      <Grid item md={6}>
                        <Typography noWrap variant="body1" className={classes.text}>
                          {paymentReceiptDetail?.customer?.ref_id ?? '-'}
                        </Typography>
                      </Grid>
                      <Grid item md={5}>
                        <Typography variant="body1" className={classes.text}>
                      ...
                      <Grid item md={6}>
                        <Typography variant="body1" className={classes.text}>
                          {paymentReceiptDetail?.customer?.name ?? '-'}
                        </Typography>
                      </Grid>
                      <Grid item md={5}>
                        <Typography variant="body1" className={classes.text}>
                      ...
                      <Grid item md={6}>
                        <Typography variant="body1" className={classes.text}>
                          {paymentReceiptDetail?.customer?.uae_phone ?? '-'}
                        </Typography>
                      </Grid>
                      <Grid item md={5}>
                        <Typography variant="body1" className={classes.text}>
                      ...
                      <Grid item md={6}>
                        <Typography variant="body1" className={classes.text}>
                          {paymentReceiptDetail?.customer?.email ?? '-'}
                        </Typography>
                      </Grid>
              ...
              <Grid container spacing={0} justifyContent={'space-between'}>
                <Grid item md={12}>
                  <Box className={classes.headingBg} sx={{ bgcolor: Colors.primary }}>
                    <Typography variant="h4" className={classes.heading}>PAYMENT</Typography>
                  </Box>
                  <Box sx={{ mr: 3, mx: 2 }}>
                    {paymentReceiptDetail?.details?.length > 0 ? (
                      <Grid container spacing={0} columns={10} justifyContent={'flex-end'}>
                        <Grid item md={8}>
                        </Grid>
                        <Grid item md={2}>
                        ...
                        <Grid item md={1}>
                          <Typography variant="subtitle2" sx={{ textAlign: 'center', fontFamily: FontFamily.NunitoRegular }}>
                            {paymentReceiptDetail?.currency?.toUpperCase()}
                          </Typography>
                        </Grid>
                        <Grid item md={1}>
                          <Typography variant="subtitle2" sx={{ textAlign: 'center', fontFamily: FontFamily.NunitoRegular }}>
                        ...
                        {paymentReceiptDetail?.details?.map((item, index) => (
                          <Fragment key={index}>
                            <Grid item md={1} className={classes.tableCell}>
                            ...
                            <Grid item md={1} className={classes.tableCell}>
                              <Typography variant="body2" className={classes.text}>
                                {paymentReceiptDetail?.created_at ? moment(paymentReceiptDetail?.created_at).format('DD-MMM-YY') : '-'}
                              </Typography>
                            </Grid>
                            <Grid item md={1} className={classes.tableCell}>
                              <Typography variant="body2" className={classes.text}>
                                {paymentReceiptDetail?.payment_medium ?? '-'}
                              </Typography>
                            </Grid>
                            <Grid item md={1} className={classes.tableCell}>
                              <Typography variant="body2" className={classes.text}>
                        ...
                        <Grid item md={1} className={classes.tableCell}>
                          ...
                          >
                            {parseFloat(paymentReceiptDetail?.amount)?.toFixed(
                              2
                            )}
                          </Typography>
                        </Grid>
                        <Grid item md={1} className={classes.tableCell}>
                          <Typography
                            ...
                            sx={{ fontWeight: 'bold !important' }}
                          >
                            {parseFloat(paymentReceiptDetail?.amount_aed)?.toFixed(
                              2
                            )}
                          </Typography>
                        </Grid>
                ...
                <Grid item md={5.5}>
                  <Box className={classes.headingBg} sx={{ pl: '32px !important', bgcolor: Colors.aliceBlue }}>
                    <Typography variant="body1" className={classes.heading} sx={{ textAlign: 'left', color: `${Colors.charcoalGrey} !important` }}><b>Processed by</b> : {paymentReceiptDetail?.user?.name} </Typography>
                  </Box>
                </Grid>
              </Grid>
              <Grid container spacing={0} justifyContent={'space-between'}>
                ...
                <Grid item md={2}>
                  <Box sx={{ height: '100%', display: 'flex', alignItems: 'center' }}>
                    <QRCodeCanvas value={window.location.origin + `/payment-receipt-preview/${id}`} />
                  </Box>
                </Grid>
              </Grid>
