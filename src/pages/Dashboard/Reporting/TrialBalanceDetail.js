...
function TrialBalanceDetailed() {
  ...
  const filterData2 = () => true // Assuming this function is defined elsewhere or always true
  ...
  filteredBalanceSheet.forEach((category) => {
    ...
    category.sub?.forEach((subCategory) => {
      ...
      subCategory.accounts?.filter(filterData2).forEach((account) => {
        ...
        if (Array.isArray(account.childAccounts)) {
          account.childAccounts.filter(filterData2).forEach((child) => {
            ...
            opening +=
              child?.nature == "credit"
                ? -1 * (Number.parseFloat(child.opening_balance) || 0)
                : Number.parseFloat(child.opening_balance) || 0
            ...
            if (Array.isArray(child.childAccounts)) {
              child.childAccounts.filter(filterData2).forEach((grandchild) => {
                ...
                opening +=
                  grandchild?.nature == "credit"
                    ? -1 * (Number.parseFloat(grandchild.opening_balance) || 0)
                    : Number.parseFloat(grandchild.opening_balance) || 0
              })
            }
          })
        }
      ...
      Object.entries(subcategoryTypeGroups).forEach(([subType, accounts]) => {
        ...
        // Initialize totals for this subcategory type group
        let totalOpening = 0
        let totalDebit = 0
        let totalCredit = 0
        let totalDiff = 0
        ...
        accounts.forEach((account) => {
          ...
          // Accumulate totals for this subcategory type group
      ...
      // Format numerical columns
      for (let i = 3; i <= 7; i++) {
        subCatTotalRow.getCell(i).numFmt = "#,##0.0000"
      }
...
const downloadExcel2 = async () => {
  ...
  filteredBalanceSheet.forEach((category) => {
    ...
    category.sub?.forEach((subCategory) => {
      ...
      Object.entries(subcategoryTypeGroups).forEach(([subType, rows]) => {
        ...
        // Format numerical columns for total row
        for (let i = 3; i <= 7; i++) {
          totalRow.getCell(i).numFmt = '#,##0.0000';
        }
  ...
  // Format numerical columns for grand total row
  for (let i = 3; i <= 7; i++) {
    grandTotalRow.getCell(i).numFmt = '#,##0.0000';
  }
                                                                                                    ...
                                                                                                    return (
                                                                                                        <React.Fragment key={account.id}>
                                                                                                            ...
                                                                                                            >
                                                                                                                ...
                                                                                                                <TableCell className="text-right">
                                                                                                                    {parseFloat(
                                                                                                                        parseFloat(parseFloat(account.nature === "debit" ? account.opening_balance : -1 * account.opening_balance ) || 0.0) +
                                                                                                                        (account.nature === "debit"
                                                                                                                            ? parseFloat(account.total_debit) - parseFloat(account.total_credit)
                                                                                                                            : -1 * (parseFloat(account.total_credit) - parseFloat(account.total_debit)))
                                                                                                                    ).toFixed(4)}
