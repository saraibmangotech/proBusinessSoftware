...
const downloadExcel = () => {
  ...
  const dateRow2 = worksheet.addRow([
    toDate && fromDate
      ? `Period:  ${moment(fromDate).format("MM/DD/YYYY")} To ${moment(toDate).format("MM/DD/YYYY")}`
      : `Period: All`
  ])
  dateRow2.getCell(1).font = { name: "Arial", size: 10, italic: true, color: { argb: "666666" } }
  ...
  filteredProfitLossStatement?.forEach((item) => {
    ...
    categoryRow.getCell(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "4472C4" } }
    categoryRow.getCell(1).font = { name: "Arial", bold: true, color: { argb: "FFFFFF" }, size: 12 }
    ...
    if (item.sub) {
      item.sub.forEach((subItem) => {
        ...
        subCategoryRow.getCell(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "E6E6FA" } }
        subCategoryRow.getCell(1).font = { name: "Arial", bold: true, size: 11, color: { argb: "2F4F4F" } }
        ...
        if (subItem.accounts) {
          subItem.accounts.forEach((account) => {
            ...
            if (account?.childAccounts?.length > 0) {
              ...
              const result = account?.childAccounts.reduce((acc, transaction) => {
                ...
                return {
                  credit: acc.credit + parseFloat(credit),
                  debit: acc.debit + parseFloat(debit),
                }
              }, initialValue)

              let periodDiff = account.nature === "debit"
                ? result.debit - result.credit
                : result.credit - result.debit

              childTotal = adjustedAccountOpeningBalance + periodDiff
              if (account.nature === "credit") childTotal *= -1
            } else {
              let diff = account.nature === "debit"
                ? parseFloat(account.total_debit || 0) - parseFloat(account.total_credit || 0)
                : parseFloat(account.total_credit || 0) - parseFloat(account.total_debit || 0)

              childTotal = adjustedAccountOpeningBalance + diff
              if (account.nature === "credit") childTotal *= -1
            }
            ...
            if (account.childAccounts) {
              account.childAccounts.forEach((child) => {
                let credit = parseFloat(child.total_credit || 0)
                let debit = parseFloat(child.total_debit || 0)

                let childPeriodDiff = child.nature === "debit"
                  ? debit - credit
                  : credit - debit

                let subTotal = 0 + childPeriodDiff
                if (child.nature === "credit") subTotal *= -1
        ...
        if (subItem.accounts?.length > 0) {
          ...
          subTotalRow.eachCell((cell, colNumber) => {
            ...
            cell.font = { name: "Arial", bold: true, size: 10, color: { argb: "2F4F4F" } }
            cell.alignment = { horizontal: colNumber > 4 ? "right" : "left", vertical: "middle" }
            cell.border = {
        ...
        if (subItem.accounts?.[0]?.type_code === "E1") {
          const grossProfit = Number(totalRevenue) - Number(totalCost)
          const grossProfitRow = worksheet.addRow(["", "", "Gross Profit", "", "", grossProfit.toFixed(2)])
          grossProfitRow.eachCell((cell, colNumber) => {
            cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "90EE90" } }
            cell.font = { name: "Arial", bold: true, size: 11, color: { argb: "2F4F4F" } }
  ...
  const netProfitRow = worksheet.addRow([
    "", "", "Net Profit", "", "", CommaSeparator((Number(totalRevenue) - Number(totalExpenses)).toFixed(2))
  ])
  netProfitRow.eachCell((cell, colNumber) => {
    cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "000000" } }
    cell.font = { name: "Arial", bold: true, color: { argb: "FFFFFF" }, size: 12 }
    cell.alignment = { horizontal: colNumber > 4 ? "right" : "center", vertical: "middle" }
    cell.border = {
  ...
  worksheet.columns = [
    { width: 15 }, { width: 35 }, { width: 20 },
    { width: 20 }, { width: 18 }, { width: 18 }
  ]
  ...
  const download = async () => {
    ...
    saveAs(blob,
      toDate && fromDate
        ? `Profit Loss Statement : ${moment(fromDate).format("MM/DD/YYYY")} To ${moment(toDate).format("MM/DD/YYYY")}`
