...
function ViewVccRefunded() {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: 'vccRefunded',
  });

  // *For Get Invoice Detail
  const getVccRefundedDetail = async () => {
    setLoader(true)
    try {
      let params = {
        vehicle_id: id
      }
      const { data } = await VccServices.getVccRefundedDetail(params)
                ...
                <Grid item md={5.5}>
                  <Box sx={{ mr: 3, mt: 5 }}>
                    <Grid container spacing={0} justifyContent={"flex-end"}>
                      ...
                      <Grid item md={6} className={classes.tableCell}>
                        <Typography variant="body1" className={classes.text}>
                          {moment(vccRefundedDetail?.vcc_refunded_at).format("DD-MMM-YYYY hh:mm")}
                        </Typography>
                      </Grid>
                    ...
                    >
                      <Typography variant="h4" className={classes.heading} sx={{ fontFamily: 'sans-serif' }}>
...
export default ViewVccRefunded;