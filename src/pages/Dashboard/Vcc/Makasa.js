...
function Makasa() {
  ...
  const tableHead = ['Customer ID', 'Customer Name', 'VIN', 'LOT', 'Make', 'Modal', 'Color', 'VCC Issue Purpose', 'VCC Declaration Number', 'VCC Declaration Date', 'VCC Expiry Date', 'Time Left', 'Makasa Status']
  ...
  const getMakasa = async (page, limit, filter) => {
    setLoader(true)
    try {
      ...
      const { data } = await VccServices.getMakasa(params)
  ...
  const renderCellContent = (colIndex, item, isActive,) => {
    ...
    switch (colIndex) {
      ...
      case 12:
        return <Box onClick={() => {
          if (
            !item?.makasa_received && !item?.is_refunded &&
  ...
  return (
    <Box sx={{ m: 4, mb: 2 }}>
      ...
      {/* ========== Receive Dialog ========== */}
      <GatePassReceiveDialog open={receiveStatusDialog} onClose={() => setReceiveStatusDialog(false)} updateStatus={() => receiveMakasa()} page={'Makasa'} />
      ...
      >
        <Typography variant="h5" sx={{ color: Colors.charcoalGrey, fontFamily: FontFamily.NunitoRegular, }}>
          Vehicle Export Certificate to GCC
        </Typography>
              ...
              vehicleList && (
                <Fragment>
                  <PDFExport ref={contentRef} landscape={true} paperSize="A4" margin={5}
                    fileName="Vehicle Export Certificate to GCC"
                  >
                  ...
                  <Pagination
                    ...
                    pageSize={pageLimit}
                    onPageSizeChange={(size) => getMakasa(1, size.target.value)}
                    tableCount={vehicleList?.length}
                    totalCount={totalCount}
                    onPageChange={(page) => getMakasa(page, "")}
                  />
