...
function VccDownloadedList() {

  const navigate = useNavigate();
  const classes = useStyles();
  const contentRef = useRef(null);

  const tableHead = ['Customer ID', 'Customer Name', 'VIN', 'VCC Declaration Number', 'Downloaded Date', 'Vcc Downloaded', 'Declaration Downloaded']
  ...
  const getVccDownloadedList = async (page, limit, filter) => {
    setLoader(true)
    try {
      ...
      const { data } = await VccServices.getVccDownloadedList(params)
  ...
  const handleFilter = (data) => {
    Debounce(() => getVccDownloadedList(1, '', data));
  }
  ...
  const renderCellContent = (colIndex, item, isActive,) => {
    ...
    switch (colIndex) {
      ...
      case 4:
        return item?.download ? moment(item?.download?.created_at).format('MM-DD-YYYY') : '-';
      case 5:
        return item?.download?.vcc_downloaded ? <Box> <span className='pdf-hide'><CheckIcon /> </span> Downloaded  </Box> : <Box> <span className='pdf-hide'> <PendingIcon /> </span> Not Downloaded  </Box>;
      case 6:
        return item?.download?.declaration_downloaded ? <Box> <span className='pdf-hide'><CheckIcon /> </span> Downloaded  </Box> : <Box> <span className='pdf-hide'><PendingIcon /> </span> Not Downloaded  </Box>;
      default:
        return "-";
    }
  };
  ...
  const downloadExcel = () => {
    ...
    const rows = vccDepositedList?.map((item) => [
      ...
      item?.download ? moment(item?.download?.created_at).format('MM-DD-YYYY') : '-',
      item?.download?.vcc_downloaded ? "Downloaded" : "Not Downloaded",
      item?.download?.declaration_downloaded ? "Downloaded" : "Not Downloaded"
    ])
              ...
              vccDepositedList && (
                <Fragment>
                  ...
                  <Pagination
                    ...
                    pageSize={pageLimit}
                    onPageSizeChange={(size) => getVccDownloadedList(1, size.target.value)}
                    tableCount={vccDepositedList?.length}
                    totalCount={totalCount}
                    onPageChange={(page) => getVccDownloadedList(page, "")}
                  />
...
export default VccDownloadedList;