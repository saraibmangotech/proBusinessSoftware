...
function IssueVcc() {
  ...
  const getVinsLots = async () => {
    try {
      ...
      let vin = data?.vins.map((item) => { return { id: item, name: item } })
      let lot = data?.lots.map((item) => { return { id: item, name: item } })
      const uniqueVins = Array.from(new Set(vin.map(item => item.id)))
        .map(id => vin.find(item => item.id === id));
      const uniqueLots = Array.from(new Set(lot.map(item => item.id)))
        .map(id => lot.find(item => item.id === id));
  ...
  const getVccVehicles = async (vin, lot) => {
    ...
    try {
      ...
      const { data: { vehicles: { rows } } } = await VccServices.getVccVehicles(params)
      const detail = rows[0]
      ...
      if (detail?.vcc?.is_approved === null) {
        ...
        setValue('vccExpDate', moment(detail?.vcc?.vcc_expiry_date).format('MM-DD-YYYY'))
        ...
        setCustomerId(detail?.booking?.customer?.id)
        setCustomerPhone(detail?.booking?.customer?.uae_phone)
        setVin(detail?.booking?.vin)
      } else if (detail?.vcc_issued) {
        ErrorToaster('VCC already issued!')
        resetData()
      } else {
        resetData()
        ErrorToaster('VCC has been approved, please issue VCC')
      }
    } catch (error) {
      ErrorToaster(error)
    } finally {
        ...
        <Grid container spacing={2} alignItems={'center'}>
          ...
          <Grid item xs={12} sm={3}>
            ...
            <SelectField
              ...
              onSelect={(value) => { setSelectedVin(value); getVccVehicles(value?.id, '') }}
            />
          </Grid>
          <Grid item xs={12} sm={3}>
            <SelectField
              ...
              onSelect={(value) => { setSelectedLot(value); getVccVehicles('', value?.id) }}
            />
          </Grid>
                ...
                <Fragment>
                  ...
                  <Grid item xs={12} sm={3}>
                    <SelectField
                      ...
                      error={errors?.vccPurpose?.message}
                      register={register("vccPurpose", {
                  ...
                  <Grid item xs={12} sm={12} sx={{ mt: 2, textAlign: 'right' }}>
                    <PrimaryButton
                      ...
                      title="Issue Vcc"
                      type='button'
                      onClick={() => selectedVccPurpose ? setIssueVccDialog(true) : ErrorToaster('Please select vcc purpose')}
                      buttonStyle={{ bgcolor: Colors.bluishCyan }}
                    />
