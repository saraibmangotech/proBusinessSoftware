...
const downloadExcel = async () => {
  ...
  worksheet.headerFooter.oddHeader =
    ...
    '&L&"Arial,Regular"&8Generated on: ' + new Date().toLocaleDateString() + '\n' +
    '&R&"Arial,Regular"&8Page &P of &N';

  worksheet.headerFooter.oddFooter =
    '&L&"Arial,Regular"&8Confidential - Internal Use Only' +
    '&C&"Arial,Regular"&8This report contains attendance data as of ' + new Date().toLocaleDateString() +
  ...
  dateRow.getCell(5).value = `Report Generated: ${new Date().toLocaleDateString('en-GB')} at ${new Date().toLocaleTimeString('en-GB', { hour12: false })}`;
  ...
  dateRow2.getCell(5).value = endDate && startDate
  ...
  const summaryData = [
    ...
    ["Overall Attendance Rate:", `${totalWorkingDays > 0 ? ((totalPresentDays / totalWorkingDays) * 100).toFixed(2) : 0}%`],
  ];
  ...
  const columnWidths = [
    { width: 25 },
    { width: 15 },
    ...Array(daysInMonth.length).fill({ width: 12 }),
  ];
  ...
  const fileName = `Attendance Report - ${
    startDate ? new Date(startDate).toLocaleDateString("en-GB") : "Start"
  } to ${
    endDate ? new Date(endDate).toLocaleDateString("en-GB") : "End"
  }.xlsx`;
