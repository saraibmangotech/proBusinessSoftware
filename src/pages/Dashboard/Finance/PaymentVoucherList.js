...
function PaymentVoucherList() {
	...
	const getVouchers = async (page, limit, filter) => {
		try {
			...
			const { data } = await FinanceServices.getVouchers(params);
	...
	const deleteVoucher = async () => {
		try {
			...
			const { message } = await FinanceServices.deleteVoucher(params);
																	...
																	>
																		{
																			item?.account?.name?.length > 12
																				? item?.account?.name?.slice(0, 8) + "..." : item?.account?.name
																		}
																	</Tooltip>
																	<Box
																		component={"div"}
