...
function UpdateFundTransferVoucher() {
    ...
    const UpdateFundTransferVoucher = async (formData) => {
        setLoading(true)
        try {
            ...
            const { message } = await FinanceServices.UpdateFundTransferVoucher(obj)
    ...
    const getData = async () => {
        try {
            ...
            const { data } = await CustomerServices.getFundTransferDetail(params);
    ...
    return (
        <Box sx={{ m: 4, p: 5, bgcolor: Colors.white, borderRadius: 3, boxShadow: '0px 8px 18px 0px #9B9B9B1A' }}>
            ...
            <Box component="form" onSubmit={handleSubmit(UpdateFundTransferVoucher)} >
                <Grid container spacing={2}>
                    <Grid item xs={12} sm={12}>
                        <Typography variant="h5" sx={{ color: Colors.charcoalGrey, fontFamily: FontFamily.NunitoRegular, mb: 4 }}>
                            Update  Internal Fund Transfer Voucher
                        </Typography>
                    </Grid>
...
export default UpdateFundTransferVoucher;