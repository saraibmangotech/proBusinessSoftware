...
function JournalVoucherList() {
  ...
  const deleteJournalVoucher = async () => {
    try {
      let params = { voucher_id: voucherId };
      const { message } = await FinanceServices.deleteJournalVoucher(params);
                        ...
                        <Fragment>
                          {vouchers.map((item, index) => (
                            <Row key={index} sx={{ bgcolor: index % 2 !== 0 && '#EEFBEE' }}>
                              ...
                              <Cell >
                                <Box component={'div'} className='pdf-hide' sx={{ display: 'flex', gap: 2 }}>
                                  <Box
                                    onClick={() =>
                                      navigate(
