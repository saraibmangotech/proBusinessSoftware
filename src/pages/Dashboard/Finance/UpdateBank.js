...
function UpdateBank() {
  ...
  const getCurrencies = async () => {
    try {
      ...
      const currency = currenciesArray.find(e => e?.id === state?.currency)
  ...
  const getCountries = async () => {
    try {
      ...
      const country = data?.nations.rows.find(e => e?.id === state?.country_id)
  ...
  const updateBank = async (formData) => {
    ...
    try {
      ...
      const { message } = await BankServices.updateBank(obj)
  ...
  useEffect(() => {
    getCurrencies()
    getCountries()
    setValue('country', state?.country_name)
    setValue('currency', state?.currency)
    setValue('bankName', state?.name)
    setValue('accountTitle', state?.account_title)
    setValue('accountNumber', state?.account_number)
    setValue('ibn', state?.account_ibn)
  }, []);

  return (
    <Box sx={{ m: 4, p: 5, bgcolor: Colors.white, borderRadius: 3, boxShadow: '0px 8px 18px 0px #9B9B9B1A' }}>

      <Box component="form" onSubmit={handleSubmit(updateBank)} >
