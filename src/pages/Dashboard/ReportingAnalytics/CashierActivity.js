...
function CashierActivity() {
  ...
  // *For Filters
  const [filters, setFilters] = useState({ nature: "vehicle" });
  ...
  const getCashierActivity = async (filter) => {
    ...
    try {
      ...
      const { data } = await FinanceServices.getCashierActivity(params);
      let Total = 0
      let totalDebit = 0
      let totalCredit = 0
      console.log(getValues('Account')?.currency, 'getValues?.currency');
  ...
  const getPaymentAccounts = async (value) => {
    try {
      ...
      const vehicleAcc = data?.cashierAccounts?.rows?.filter(
        (e) => e.unit === getValues('Nature').name
      );
