...
function CustomerPaymentList() {
    ...
    const downloadCustomerPaymentsExcel = () => {
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet("Customer Payments");
        ...
        const titleRow = worksheet.addRow(["CUSTOMER PAYMENTS"]);
        ...
        const download = async () => {
            ...
            saveAs(blob,
                fromDate && toDate
                    ? `Customer_Payments_${moment(fromDate).format("MM-DD-YYYY")}_To_${moment(toDate).format("MM-DD-YYYY")}.xlsx`
                    : `Customer_Payments_${moment().format("MM-DD-YYYY")}.xlsx`
            );
        };
        download();
    };
                    ...
                    <Box component={'div'} className='pdf-hide' sx={{ gap: '16px !important' }}>
                       <Tooltip title="View Invoice">
                        <IconButton
                             onClick={() => {
                                window.open(
                                    `${process.env.REACT_APP_INVOICE_GENERATOR}generate-customer-voucher?id=${row?.original?.id}&instance=${process.env.REACT_APP_TYPE}`,
                                    '_blank'
                                );
                            }}
                            sx={{
                ...
                {data?.length > 0 && (
                    <Box sx={{
                        ...
                        <PrimaryButton
                            ...
                            onClick={() => downloadCustomerPaymentsExcel()}
                        />
                    </Box>
                )}
                {/* {true && (
            ...
            <Grid container spacing={1} justifyContent={"space-between"} alignItems={"center"}>
                ...
                <Grid item xs={4} display={'flex'} mt={2.7} justifyContent={'flex-end'}>
                    <PrimaryButton
                        ...
                        onClick={() => {
                            navigate("/create-customer-payment");
                            localStorage.setItem("currentUrl", "/create-customer");
                        }}
                        loading={loading}
                    />
