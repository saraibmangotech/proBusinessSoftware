...
function ContainerPayment() {
        ...
        switch (colIndex) {
            ...
            case 7:
                return <b>
                    USD{" "}
                    {
                        item?.paid_amount ? parseFloat(item?.paid_amount).toFixed(2) : parseFloat(0).toFixed(2)}
                </b>;
            case 8:
                return <b>
                    USD{" "}
    ...
    const downloadExcel = () => {
        ...
        const rows = clientCosting?.map((item) => [
            ...
            item?.customer?.name ?? "-",
            item?.paid_amount == 0
                ? "Unpaid" : parseFloat(item?.paid_amount) > 0 && parseFloat(item?.paid_amount) < parseFloat(item?.price)
                    ? "Partial"
                    : "Paid",
            parseFloat(item?.price).toFixed(2) ?? "-",
            0.0 ?? "-",
            `$ ${item?.paid_amount ? parseFloat(item?.paid_amount).toFixed(2) : parseFloat(0).toFixed(2)}`,
            `USD ${balances[item?.id] ?? parseFloat((item?.price) - parseFloat(item?.paid_amount)).toFixed(2)}`
        ])
        const totalRow = [
            "",
            "",
