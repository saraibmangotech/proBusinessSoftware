		...
		try {
			...
			if (data?.vehicles?.rows[0]?.agent && data?.vehicles?.rows[0]?.broker) {
				...
				if (duplicates.length > 0) {
					...
					console.log("Setting Export Data:", list); // Added console.log
					setValue('ManifestNumber', list[0]?.manifest_number)
					handleManifestDate(list[0]?.manifest_date)
					let manifestNumbers = [...ExportData, ...newRows].map(item => item?.manifest_number);
					console.log(manifestNumbers, 'manifestNumbers');
					let newManifests = []
			...
			>
				<Grid container spacing={4}>
					<Grid item xs={12} sm={12} md={12} lg={6}>
						<Grid container>
							...
							<Grid item sm={6}>
								<InputField
									...
									disabled={disabled}
									size={"small"}
									error={errors?.ManifestNumber?.message}
									register={register("ManifestNumber", {
						...
						{/* Repeat similar structure for Manifest Date, CMR Number, and CMR Date */}
					</Grid>
				...
				<Grid item xs={12} sm={12} sx={{ mt: 4, textAlign: "right", width: "98%" }}>
					<PrimaryButton type="submit" title="Save & Update" loading={loading} />
				</Grid>
