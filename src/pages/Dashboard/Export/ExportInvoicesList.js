...
function ExportVehiclePayments() {
	...
	const downloadExcel = () => {
		...
		const rows = ExportDatas?.map((item) => {
			const vins = (item?.details || []).map(detail => detail?.vehicle?.vin).join(',');
			return [
				item?.id ?? "-",
													...
													{ExportDatas.map((item, index) => {
														...
														return (
															...
															>
																...
																<Cell className="pdf-table">{item?.details ? item?.details?.length : 0 ?? "-"}</Cell>
																<Cell className="pdf-table">
																	<Tooltip
																		className="pdf-hide"
																		title={vins ?? "-"}
																	...
																	>
																		{vins.length > 16 ? vins.slice(0, 16) + "..." : vins}
																	</Tooltip>
																	<Box
																		component={"div"}
																		className='pdf-show'
																...
																<Cell className="pdf-table">{(item?.creator?.name) ?? "-"}</Cell>
																<Cell>
																	<Box component={'div'}
																		className="pdf-hide"
																		onClick={() =>
																			navigate(`/export-payment-receipt/${item?.id}`, {
																				state: {
																					shipping: true,
																				},
																			})
