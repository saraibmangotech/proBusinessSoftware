...
function ExportTTDetail() {
  ...
  const getTTDetail = async () => {
    ...
    try {
      ...
      const { data } = await ExportServices.getTTDetail(params)
              ...
              <Grid container spacing={2} alignItems='flex-start'>
                ...
                <Grid container item xs={12} sm={12} justifyContent={'space-between'}>
                  ...
                  <Grid item sm={4}>
                    ...
                    <Box sx={{ display: 'flex', gap: '5px', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <Typography variant="body1">
                        {parseFloat(ttDetail?.vat_charges).toFixed(2)}
                      </Typography>
                    </Box>
                  </Grid>
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', mr: 4 }}>
                    <QRCodeCanvas value={window.location.origin + `/export-tt-detail-preview/${btoa('tt-' + id)}`} />
                  </Box>
                </Grid>
              </Grid>
            </Box>
