                        ...
                        <Fragment>
                          {vendorTT.map((item, index) => (
                            <Row key={index} sx={{ bgcolor: index % 2 !== 0 && '#EEFBEE' }}>
                              ...
                              <Cell className="pdf-table" >
                                {CommaSeparator(item?.fcy_amount) ?? '-'}
                              </Cell>
                              <Cell className="pdf-table" >
                                {CommaSeparator(item?.lcy_amount) ?? '-'}
                              </Cell>
                              <Cell className="pdf-table" >
                                {CommaSeparator(item?.tt_charges) ?? '-'}
                              </Cell>
                              <Cell className="pdf-table" >
                                {CommaSeparator(item?.vat_charges) ?? '-'}
                              </Cell>
                              <Cell className="pdf-table" >
                                {CommaSeparator(item?.total_amount) ?? '-'}
                              </Cell>
                              <Cell className="pdf-table" >
                                {CommaSeparator(item?.paid_amount) ?? '-'}
                              </Cell>
                              <Cell className="pdf-table" >
                                {CommaSeparator(parseFloat(parseFloat(item?.total_amount) - parseFloat(item?.paid_amount)).toFixed(2)) ?? '-'}
                              </Cell>
                              <Cell className="pdf-table" >
                                {item?.is_paid ?? '-'}
                                <Box>
                              ...
                              <Cell>
                                <Box component={'div'}
                                  ...
                                  <Box onClick={() => navigate(`/export-tt-detail/${item?.id}`)}>
