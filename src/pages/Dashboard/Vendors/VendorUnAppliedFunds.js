...
import { EyeIcon, FontFamily, PendingIcon } from "assets";
...
function VendorUnAppliedFunds() {
	const classes = useStyles();
	const contentRef = useRef(null);
	const navigate = useNavigate();
	const dispatch = useDispatch();
	const tableHead = ["TT Date", "Ref No", "External Ref No", "Toal Amount", "Paid Amount", "Status", "Action"];
	...
	const downloadExcel = () => {
		...
		const rows = vendorTT?.map((item) => [
			...
			`VUAF-${item?.id ?? "-"}`,
			item?.external_no ?? "-",
			parseFloat(item?.fcy_amount).toFixed(2) ?? "-",
			parseFloat(parseFloat(item?.fcy_amount) / parseFloat(item?.ex_rate)).toFixed(2) ?? "-",
			item?.approval_pending
				? "In Progress"
				: item?.paid_amount == 0
					? "Unpaid"
					: (parseFloat(item?.paid_amount) / parseFloat(item?.ex_rate) < parseFloat(item?.fcy_amount))
						? "Partial"
						: "Paid"
		])
					...
					vendorTT && (
						<Fragment>
							<PDFExport ref={contentRef} landscape={true} paperSize="A4" margin={5}
								fileName="Vendor Un-Applied Funds"
							>
													...
													<Fragment>
														{vendorTT.map((item, index) => (
															...
															>
																...
																<Cell className="pdf-table">{parseFloat(item?.fcy_amount).toFixed(2) ?? "-"}</Cell>
																<Cell className="pdf-table">{parseFloat(parseFloat(item?.fcy_amount) / parseFloat(item?.ex_rate)).toFixed(2) ?? "-"}</Cell>
																<Cell>
																	...
																	{item?.approval_pending ?
																		<Box>
																			...
																			>
																				<PendingIcon color={Colors.yellow} sx={{
																					fontSize: "18px",
																					color: '#25ABE1',
																			...
																			<span className="pdf-table">		In Progress </span>
																		</Box>
																		:
...
export default VendorUnAppliedFunds;
