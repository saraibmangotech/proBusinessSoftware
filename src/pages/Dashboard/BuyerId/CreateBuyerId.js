...
function CreateBuyerId() {
  ...
  // *For Create Buyer ID
  const createBuyerId = async (formData) => {
    setLoading(true)
    try {
      let obj = {
        ...
        auction_account_id: selectedAuctionHouses?.account_id,
      }
      const { message } = await BuyerServices.createBuyerId(obj)
  ...
  return (
    <Box sx={{ m: 4, p: 5, bgcolor: Colors.white, borderRadius: 3, boxShadow: '0px 8px 18px 0px #9B9B9B1A' }}>
      ...
      <Box component="form" onSubmit={handleSubmit(createBuyerId)} >
        <Grid container spacing={0} alignItem={'center'} justifyContent={'center'}>
          ...
          <Grid container spacing={1} item xs={12} sm={9}>
            ...
            <Grid item xs={12} sm={6}>
              <SelectField
                label={'Category'}
                options={[{ id: 'N/A', name: 'N/A' }, { id: 'A', name: 'Category A' }, { id: 'B', name: 'Broker' }, { id: 'C', name: 'Category C' }]}
