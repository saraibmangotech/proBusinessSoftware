...
function ViewInvoice() {
	...
	const getInvoiceDetails = async () => {
		...
		try {
			...
			const { data } = await VehiclePaymentServices.getInvoiceDetails(params)
			...
			>
				{!loader ? (
					<PDFExport ref={contentRef}
						fileName="Vehicle Invoice"
					>
						<Box className={`pdf-page ${layoutSelection.value}`}>
								...
								<Grid item xs={5.5} sm={5.5} md={5.5}>
									<Box sx={{ ml: 4, mb: 7.5 }}>
										<Grid container spacing={0.5}>
											...
											<Grid item xs={6} sm={6} md={6}>
												<Typography variant="body1" className={classes.text}>
													{invoiceDetail?.invoice?.booking?.location
														?.country_name ?? "-"}
												</Typography>
											</Grid>
										</Grid>
									...
									<Box sx={{ ml: 4 }}>
										<Grid container spacing={1.34}>
											...
											<Grid item xs={6} sm={6} md={6}>
												<Typography variant="body1" className={classes.text}>
													{moment(
														invoiceDetail?.invoice?.booking?.purchase_date
									...
									<Box sx={{ ml: 4, mr: 2 }}>
										<Typography variant="body1" className={classes.text}>
											{invoiceDetail?.pay_history?.rows?.map(
												(item, index) => `${item?.payment?.comments}`
											)}
										</Typography>
									</Box>
									<Box
											...
											>
												{invoiceDetail?.pay_history?.rows?.map(
													(item, index) =>
														...
														} received by ${item?.payment?.user?.name}, `
												)}
											</Typography>
										</Typography>
									</Box>
									...
									<Box sx={{ mr: 3, mb: 8 }}>
										<Grid container spacing={0} justifyContent={"flex-end"}>
											...
											>
												...
												>
													{parseFloat(
														invoiceDetail?.invoice?.booking?.value
													)?.toFixed(2)}
												</Typography>
											</Grid>
											<Grid
											...
											>
												...
												>
													{parseFloat(
														invoiceDetail?.invoice?.booking?.other_charges
													)?.toFixed(2)}
												</Typography>
											</Grid>
											<Grid
											...
											>
												<Typography
													...
													sx={{ textAlign: "right" }}
												>
													{parseFloat(invoiceDetail?.invoice?.paid)?.toFixed(
														2
													)}
												</Typography>
											</Grid>
													...
													<Fragment key={index}>
														...
														>
															<Typography
																...
																sx={{ textAlign: "right" }}
															>
																{parseFloat(item?.amount_aed)?.toFixed(
																	2
																)}
															</Typography>
														</Grid>
									...
									<Box sx={{ height: "100%", display: "flex", alignItems: "center" }}>
										<QRCodeCanvas
											value={
												...
												`/invoice-preview/${btoa("vehiclepayment-" + id)}`
											}
										/>
									</Box>
								</Grid>
