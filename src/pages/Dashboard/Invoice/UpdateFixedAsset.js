...
function UpdateFixedAssetInvoice() {
        ...
        if (existingTotal <= newTotal) {
            ...
            try {
                ...
                if (detail?.is_paid == true) {
                    ...
                    const promise = FPInvoiceServices.updateFPInvoice(obj)
    ...
    const getData = async (array) => {
        try {
            ...
            const { data } = await CustomerServices.getPrepaidInvoiceDetail(params);
            ...
            setValue1('amortization_months', detail?.total_months)
    ...
    useEffect(() => {
        ...
        setValue1('depreciation', find)
        let find3 = accounts3?.find((item => item?.id == detail?.credit_account_id))
        console.log(find, 'find');
        setSelectedAccount3(find)
        setValue1('accumulated', find3)
    }, [detail, accounts2, accounts3])
                            ...
                            <Grid container spacing={2} p={2}>
                                ...
                                <Grid item md={3} sm={5.5} xs={12}>
                                    <SelectField
                                        ...
                                        error={errors1?.depreciation?.message}
                                        register={register1("depreciation", {
...
export default UpdateFixedAssetInvoice;
