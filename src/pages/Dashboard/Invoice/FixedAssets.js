...
function PrepaidInvoices() {
  ...
  // *For Get Customer Queue
    ...
    const getCustomerQueue = async (page, limit, filter) => {
      ...
      try {
        ...
        const { data } = await CustomerServices.getFixedAssets(params)
  ...
  const handleDelete = async (item) => {
    ...
    try {
        let params = { id: selectedData?.id ,account_id:selectedData?.account_id}


        const { message } = await CustomerServices.deleteFixedAssets(params)
     ...
     
      {
        ...
        cell: ({ row }) => (
          ...
          <Box sx={{display:'flex',gap:1}}>
            ...
            {row?.original?.months_recorded == 0 && <Box component={'img'} sx={{ cursor: "pointer" }} onClick={() => { navigate(`/update-fixed-assets/${row?.original?.id}`); localStorage.setItem("currentUrl", '/update-prepaid-invoice') }} src={Images.editIcon} width={'35px'}></Box>}
            <Box>
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {user?.role_id != 1003 && <PrimaryButton
         bgcolor={'#001f3f'}
          title="Create "
          onClick={() => { navigate('/create-fixed-assets'); localStorage.setItem("currentUrl", '/create-fixed-assets') }}
          loading={loading}
        />}
