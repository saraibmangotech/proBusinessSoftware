...
function EmployeePaySlips() {
    ...
    const getCustomerQueue = async (page, limit, filter) => {
        ...
        try {
            ...
            const { data } = await CustomerServices.getPaySlips(params)
            setCustomerQueue(data?.payslips?.rows)
    ...
    const columns = [
        ...
        },
        {
            ...
            accessorKey: "month",
            cell: ({ row }) => {
                const monthNumber = row?.original?.employee_salary?.month;
                const monthName = moment().month(monthNumber - 1).format("MMMM"); // Subtract 1 since moment uses 0-indexed months
                return (
                    <Box
                        variant="contained"
                        color="primary"
        ...
        ,
        {
            ...
            cell: ({ row }) => (
                <Box variant="contained" color="primary" sx={{ cursor: "pointer", display: "flex", gap: 2 }}>
                    {row?.original?.employee_salary?.year
                    }
                </Box>
            ),
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                    <Tooltip title="Slip">
                        <IconButton
                            onClick={() => {
                                window.open(
                                    `${process.env.REACT_APP_INVOICE_GENERATOR}generate-payslip?id=${row?.original?.id}&instance=${process.env.REACT_APP_TYPE}`,
                                    '_blank'
                                );
                            }}
                            sx={{
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography sx={{ fontSize: '24px', fontWeight: 'bold' }}>Employee Pay Slips</Typography>
