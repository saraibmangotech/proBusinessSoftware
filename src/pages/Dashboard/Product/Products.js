    ...
    const columns = [
...

        {
            ...
            cell: ({ row }) => (
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                   {true && <Box component={'img'} sx={{ cursor: "pointer" }} onClick={() => { navigate(`/product-detail/${row?.original?.id}`); localStorage.setItem("currentUrl", '/customer-detail'); }} src={Images.detailIcon} width={'35px'}></Box>}
                    {true && <Box component={'img'} sx={{ cursor: "pointer" }} onClick={() => { navigate(`/update-product/${row?.original?.id}`); localStorage.setItem("currentUrl", '/update-customer') }} src={Images.editIcon} width={'35px'}></Box>}
                    <Box>
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography sx={{ fontSize: '24px', fontWeight: 'bold' }}>Product List</Typography>
                {true && <PrimaryButton
                    bgcolor={'#bd9b4a'}
                    title="Create"
