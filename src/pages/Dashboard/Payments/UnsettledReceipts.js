...
function UnsettledReceipts() {
    ...
    const UnsettledInvoice = async (item) => {
        ...
        try {
            ...
            const { message } = await CustomerServices.RefundInvoice(params)
    ...
    const columns = [
        ...
        
        {
            id: "status",
            header: "Status",
            accessorFn: (row) => row?.is_refunded,
            cell: ({ row }) => (
                <Box variant="contained" color="primary" sx={{ cursor: "pointer", display: "flex", gap: 2 }}>
                    {row?.original?.is_refunded ? 'Refunded' : 'Pending'}
                </Box>
            ),
        },
        {
                ...
                <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
                    ...
                    {!row?.original?.is_refunded && (
                        <Tooltip title="Refund">
                            ...
                            >
                                <ReplayIcon color="black" fontSize="small" />
                            </IconButton>
                        </Tooltip>
                    )}
    ...
    const downloadPaidReceiptsExcel = () => {
        ...
        const worksheet = workbook.addWorksheet("Unsettled Receipts")
        ...
        const titleRow = worksheet.addRow(["UNSETTLED RECEIPTS"])
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography sx={{ fontSize: '24px', fontWeight: 'bold' }}>Unsettled Receipt List</Typography>
...
export default UnsettledReceipts;