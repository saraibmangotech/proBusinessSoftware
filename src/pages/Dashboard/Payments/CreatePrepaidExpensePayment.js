...
function CreatePrepaidExpensePayment() {
    ...
    const receivePayment = async (formData) => {
        ...
        try {
            ...
            const promise = FPInvoiceServices.addFPInvoicePayments(obj)
            const response = await promise
            showPromiseToast(promise, "Saving...", "Added Successfully", "Something Went Wrong")
            if (response?.responseCode === 200) {
                navigate('/prepaid-expense-payments')
            }
...
export default CreatePrepaidExpensePayment;