...
function PurchaseInvoicePaymentList() {
    ...
    const getCustomerQueue = async (id) => {
        ...
        try {
            ...
            const { data } = await FPInvoiceServices.getFPInvoicePaymentHistory(params)
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                    ...
                    <Tooltip title="Invoice">
                        <IconButton
                            onClick={() => {
                                window.open(
                                    `${process.env.REACT_APP_INVOICE_GENERATOR}generate-fp-invoice?id=${row?.original?.invoice_id}&instance=${process.env.REACT_APP_TYPE}`,
                                    '_blank'
                                );
                            }}
                            sx={{
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography sx={{ fontSize: '24px', fontWeight: 'bold' }}>  Payment History</Typography>
