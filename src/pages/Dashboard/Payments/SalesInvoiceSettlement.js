...
function SalesInvoiceSettlement() {
    ...
    const receivePayment = async (formData) => {
        ...
        try {
            let obj = {
                customer_id: invoiceList[0]?.customer_id,
                customer_name: invoiceList[0]?.customer?.name,
            ...
            // const promise = VehiclePaymentServices.SalesInvoiceSettlement(obj)
    ...
    useEffect(() => {
        if (selectedInvoice2.length > 0) {
            ...
            const totalAmountUnpaid = selectedInvoice2.reduce((total, invoice) => total + (invoice.amount || 0), 0);
