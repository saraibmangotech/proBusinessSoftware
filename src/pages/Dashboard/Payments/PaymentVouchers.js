...
function PaymentVouchers() {
    ...
    worksheet.mergeCells(`A${poweredByRow.number}:H${poweredByRow.number}`)
        ...
        const download = async () => {
            ...
            saveAs( blob,
                toDate && fromDate
                  ? `Payment_Vouchers : ${fromDate ? new Date(fromDate).toLocaleDateString('en-GB', {day: '2-digit', month: '2-digit', year: 'numeric'}) : "-"} To ${toDate ? new Date(toDate).toLocaleDateString('en-GB', {day: '2-digit', month: '2-digit', year: 'numeric'}) : "Present"}`
