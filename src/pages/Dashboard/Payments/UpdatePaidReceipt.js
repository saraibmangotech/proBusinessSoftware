...
function UpdatePaidReceipt() {
    ...
    const getData = async () => {
        try {
            ...
            const { data } = await CustomerServices.getPaidReceiptDetail(params);

            console.log(data);
            let detail = data?.receipt
            setDate(new Date(detail?.paid_date))
            setValue1('invoiceID', detail?.invoice_number)
            setSelectedCustomer(detail?.customer)
            setValue1('display_customer', detail?.customer?.name)
            setValue1('mobile', detail?.customer?.mobile)
            setValue1('email', detail?.customer?.email)
            ...
            <Box m={3} sx={{ backgroundColor: "white", borderRadius: "12px" }}>
                {
                    <>
                        <Box component={"form"} onSubmit={handleSubmit1(submitForm1)}>
                            ...
                            >
                                <Typography sx={{ fontSize: "22px", fontWeight: "bold" }}>Update Payment Method</Typography>
                            </Box>
