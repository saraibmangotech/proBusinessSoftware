...
function VatInputRegister() {
  ...
  const handleExcelDownload = () => {
    ...
    const worksheet = workbook.addWorksheet("Customer Queue")
  
    // Set professional header
    worksheet.headerFooter.oddHeader =
      '&C&"Arial,Bold"&18VAT OUTPUT REGISTER\n' +
    ...
    const titleRow = worksheet.addRow(["VAT INPUT REGISTER"])
    ...
    // Add TOTAL row exactly as in your original code
    ...
    // Add custom total rows from data object exactly as in your original code
    if (data && Object.keys(data).length > 0) {
      // Add empty row for spacing
      worksheet.addRow([])
  
      // Add custom total rows
      Object.entries(data).forEach(([key, value]) => {
        const label = key.replace(/total/, "Total ")
        ...
        // Style custom total rows
        customRow.getCell(1).font = { name: "Arial", size: 11, bold: true }
        customRow.getCell(1).alignment = { horizontal: "left", vertical: "middle" }
  
        customRow.getCell(2).font = { name: "Arial", size: 11, bold: true }
    ...
    workbook.properties = {
      ...
      subject: "VAT Report",
      keywords: "vat, output, register, tax, financial",
      category: "VAT Reports",
      description: "VAT output register generated from accounting system",
      company: companyName,
    }
    ...
    const download = async () => {
      ...
      const blob = new Blob([buffer], {
        type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
           ...
           saveAs(blob,
                      toDate && fromDate
                          ? `vat_input_register : ${fromDate ? new Date(fromDate).toLocaleDateString('en-GB', {day: '2-digit', month: '2-digit', year: 'numeric'}) : "-"} To ${toDate ? new Date(toDate).toLocaleDateString('en-GB', {day: '2-digit', month: '2-digit', year: 'numeric'}) : "Present"}`
...
export default VatInputRegister;