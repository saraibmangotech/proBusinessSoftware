...
function AgentViseVisaList() {
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      const csvHeaders = [
        ...
        { label: "Visa Fee As per Agreement", key: "commission_monthly" },
        { label: "Visa Cost ", key: "commission_monthly" },
        { label: "PRO Typing Fee", key: "commission_monthly" },
        { label: "Mabde  ", key: "commission_monthly" },
        { label: "Agent Fee ", key: "commission_monthly" },
        { label: "Net Profit MABDE", key: "commission_monthly" },
      ];
      ...
      const csvData = data?.rows?.map((item, index) => {
        let foundStatus = item?.statuses.find(
          ...
          if (
            ...
            charges?.visa_tenure === "1 year" &&
            charges?.visa_type === "Out"
          ) {
            VisaCost =
              parseFloat(charges?.one_year_outside) +
              parseFloat(charges?.one_year_renewal);
          } else if (
            charges?.visa_tenure === "2 year" &&
            charges?.visa_type === "In"
          ) {
            VisaCost =
        ...
        return {
          ...
          "Status Date":item?.change_status_date
                                            ? moment(
                                                item?.change_status_date
                                              ).format("MM-DD-YYYY")
                                            : "-",
          "Visa Fee As per Agreement": item?.visa_charges || "-",
          "Visa Cost": item?.visa_cost,
          "PRO Typing Fee": charges?.typing_fee,
          "Mabde": isNaN(profit.toFixed(2))
          ? "-"
          : profit.toFixed(2),
          "Agent Fee": isNaN(agentFee.toFixed(2))
          ? "-"
          : agentFee.toFixed(2),
          "Net Profit MABDE": isNaN(netMabde.toFixed(2))
          ? "-"
          : netMabde.toFixed(2),
        };
      });
  ...
  useEffect(() => {
    ...
    if (user?.user_type == "A") {
      ...
      getVisaSales(null, null, null, user?.agent_id);
    }
  }, []);
...
export default AgentViseVisaList;
