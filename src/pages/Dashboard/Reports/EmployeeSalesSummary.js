...
function EmployeeSalesSummary() {
    ...
    const downloadEmployeeSalesSummaryExcel = (data) => {
        ...
        const download = async () => {
            ...
            saveAs(blob,
                toDate && fromDate
                    ? `employee_sales_summary_report : ${fromDate ? moment(fromDate).format("DD/MM/YYYY") : "-"} To ${toDate ? moment(toDate).format("DD/MM/YYYY") : "Present"}`
        ...
        <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                ...
                {customerQueue?.length > 0 &&
                    <Button
                        onClick={() => downloadEmployeeSalesSummaryExcel(customerQueue)}
