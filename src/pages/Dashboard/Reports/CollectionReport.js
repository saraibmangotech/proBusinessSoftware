   ...
   agencyType[process.env.REACT_APP_TYPE]?.name
    ...
    customerQueue.forEach((row, index) => {
      const rowData = columns.map((col, colIndex) => {
        ...
        if (col.accessorKey === "receipt_date") {
          console.log(value,'valuevaluevalue');
          
          value = new Date(moment(value,"DD/MM/YYYY").format('YYYY-MM-DD'))
        }
      ...
      dataRow.eachCell((cell, colNumber) => {
        ...
        // Format numeric cells - Fixed to use numbers instead of strings
    ...
    // Add TOTAL row - Fixed to use numbers instead of strings
    const totalRowData = columns.map((col, i) => {
      if (i === 0) return "TOTAL";
      const val = totals[col.header];
      return val != null ? val : ""; // Return number directly, not string
    });
    ...
    if (data && Object.keys(data).length > 0) {
      ...
      // Add custom total rows in a structured format - Fixed to use numbers
      Object.entries(data).forEach(([key, value]) => {
        ...
        customRowData[1] = Number.parseFloat(value || 0); // Use number directly
    ...
    const download = async () => {
      ...
      saveAs(blob,
        toDate && fromDate
          ? `Collection_Report : ${fromDate ? new Date(fromDate).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }) : "-"} To ${toDate ? new Date(toDate).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }) : "Present"}`
