...
function AgingReportPayable() {
    ...
    const getCustomerQueue = async (page, limit, filter) => {
        ...
        try {
            ...
            const { data } = await CustomerServices.getAgingReportPayable(params)
            setCustomerQueue(data?.vendors)
    ...
    const downloadCategoryReportExcel = (data) => {
        ...
        const worksheet = workbook.addWorksheet("Aging Report Payable");

        // Header/Footer & Page setup code remains unchanged...

        // --- Add Title Rows ---
        const titleRow = worksheet.addRow(["AGING REPORT PAYABLE"]);
        ...
        const download = async () => {
            ...
            saveAs(
                blob,`aging_report_payable.xlsx`
            );
        };
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography sx={{ fontSize: '24px', fontWeight: 'bold' }}> Aging Report Payable</Typography>
...
export default AgingReportPayable;