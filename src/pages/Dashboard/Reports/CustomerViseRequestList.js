...
function CustomerViseRequestList() {
  const {
    register,
    handleSubmit,
    getValues,
    ...
    try {
      ...
      const csvData = data?.rows?.map((item, index) => {
        ...
        //     : "-",
          ...
          "Visa Cost (AED)": item?.visa_cost,
          "Monthly Charges (AED)": charges?.typing_fee,
          "Agent Fee (AED)": isNaN(profit.toFixed(2))
          ? "-"
          : profit.toFixed(2),
          "Net Profit MABDE (AED)": isNaN(agentFee.toFixed(2))
          ? "-"
          : agentFee.toFixed(2),
          "Fahad Share (AED)": isNaN(netMabde.toFixed(2))
          ? "-"
          : netMabde.toFixed(2),
          ...
          {visas.length > 0 && (
            <>
              ...
              <CSVLink
                ref={csvLink}
                data={csvData}
                filename={`Customer_Vise_Request_list${moment().format(
                  "DD-MMM-YYYY_HHmmss"
                )}.csv`}
                target="_blank"
              ></CSVLink>
...
export default CustomerViseRequestList;
