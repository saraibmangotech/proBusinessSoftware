...
function EmployeeWiseSalesReport() {
    ...
    const downloadEmployeeWiseSalesReportExcel = () => {
        // Skip if no data
        if (!customerQueue || customerQueue.length === 0) return

        const workbook = new ExcelJS.Workbook()
        const worksheet = workbook.addWorksheet("Employee Wise Sales Report")

        // Set professional header
        worksheet.headerFooter.oddHeader =
            '&C&"Arial,Bold"&18EMPLOYEE WISE SALES REPORT\n' +
        ...
        const titleRow = worksheet.addRow(["EMPLOYEE WISE SALES REPORT"])
        ...
        customerQueue.forEach((row, index) => {
            const dataRow = worksheet.addRow([
                row?.category || "",
                row?.itemCount || 0,
                Number.parseFloat(row?.totalCharges || 0).toFixed(2),
            ...
            totals.totalCharges = (totals.totalCharges || 0) + Number.parseFloat(row?.totalCharges || 0)
        ...
        const totalRow = worksheet.addRow([
            "TOTAL",
            totals.count || 0,
            totals.totalCharges?.toFixed(2) || "0.00",
        ...
        workbook.properties = {
            ...
            subject: "Employee Sales Analysis Report",
            keywords: "employee, sales, category, analysis, financial, charges, commission",
            category: "Sales Reports",
            description: "Employee wise sales report generated from accounting system",
            company: companyName,
        }
        ...
        const download = async () => {
            ...
            saveAs(blob,
                toDate && fromDate
                    ? `employee_wise_sales_report : ${fromDate ? new Date(fromDate).toLocaleDateString('en-GB', {day: '2-digit', month: '2-digit', year: 'numeric'}) : "-"} To ${toDate ? new Date(toDate).toLocaleDateString('en-GB', {day: '2-digit', month: '2-digit', year: 'numeric'}) : "Present"}`
