...
function EOSReport() {
  ...
  const [filteredData, setFilteredData] = useState(eosReportData)
  ...
  const handleExcelExport = () => {
    const exportData = filteredData.map((emp) => ({
      ...
      "Joining Date": moment(emp.joining_date).format("DD/MM/YYYY"),
      "Leaving Date": moment(emp.leaving_date).format("DD/MM/YYYY"),
      "Years of Service": emp.years_of_service,
      "Months of Service": emp.months_of_service,
      "Last Basic Salary (AED)": emp.last_basic_salary.toLocaleString(),
      "Total Salary (AED)": emp.total_salary.toLocaleString(),
      "Gratuity Amount (AED)": emp.gratuity_amount.toLocaleString(),
      Status: emp.status,
    }))

    const worksheet = XLSX.utils.json_to_sheet(exportData)
    const workbook = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(workbook, worksheet, "EOS Report")
    ...
    saveAs(data, `EOS_Report_${moment().format("YYYY-MM-DD")}.xlsx`)
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => <Box>{row.original.years_of_service.toFixed(2)} years</Box>,
    },
    {
      header: "Last Basic Salary",
      accessorKey: "last_basic_salary",
      cell: ({ row }) => <Box>AED {row.original.last_basic_salary.toLocaleString()}</Box>,
    },
    {
      header: "Total Salary",
      accessorKey: "total_salary",
      cell: ({ row }) => <Box>AED {row.original.total_salary.toLocaleString()}</Box>,
    },
    {
        ...
        <Grid item xs={12} sm={6} md={3}>
          ...
          >
            ...
            <Typography sx={{ fontSize: "20px", fontWeight: "bold", color: "#1b5e20" }}>
              AED {totalGratuity.toLocaleString()}
            </Typography>
          </Box>
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          ...
          >
            ...
            <Typography sx={{ fontSize: "20px", fontWeight: "bold", color: "#e65100" }}>
              AED {totalEmployees > 0 ? (totalGratuity / totalEmployees).toLocaleString() : "0"}
            </Typography>
          </Box>
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          ...
          >
            ...
            <Typography sx={{ fontSize: "24px", fontWeight: "bold", color: "#ad1457" }}>
              {filteredData.filter((emp) => emp.status === "Pending").length}
            </Typography>
          </Box>
        </Grid>
      </Grid>
