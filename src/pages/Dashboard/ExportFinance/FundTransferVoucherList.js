...
function FundTransferVoucherList() {
  ...
  const getFundTransferVouchers = async (page, limit, filter) => {
    try {
      ...
      const { data } = await ExportFinanceServices.getFundTransferVouchers(params);
  ...
  const renderCellContent = (colIndex, item, isActive,) => {
    ...
    switch (colIndex) {
      ...
      case 1:
        return (
          <Box>
            ...
            >
              {item?.ref_no?.length > 14 ? item?.ref_no?.slice(0, 14) + "..." : item?.ref_no}
            </Tooltip>
            <Box
              component={"div"}
              className="pdf-show"
      ...
      case 6:
        return CommaSeparator(parseFloat(item?.from_amount).toFixed(2));
      case 7:
        return CommaSeparator(parseFloat(item?.exchange_rate).toFixed(3));
      case 8:
        return CommaSeparator(parseFloat(item?.to_amount).toFixed(2));
      case 9:
        return CommaSeparator(parseFloat(item?.exchange_loss).toFixed(2));
      case 10:
      ...
      case 13:
        return <Box component={'div'} className="pdf-hide" sx={{ gap: '16px !important' }}>
          <Box
            onClick={() =>
              navigate(
                `/fund-transfer-voucher-detail-export/${item?.id}`
              )
            }
          >
            <IconButton
