...
function AccountLedger() {
  ...
  console.log("🚀 ~ AccountLedger ~ accountLedgers:", accountLedgers)
  ...
  const downloadExcel = () => {
    ...
    const rows = accountLedgers?.map((item) => {
      ...
      const balance = state?.nature === 'debit' ? (parseFloat(item?.debit) - parseFloat(item?.credit)).toFixed(2) : (parseFloat(item?.credit) - parseFloat(item?.debit)).toFixed(2)
    ...
    <Box sx={{ m: 4, mb: 2 }}>
      ...
      >
        <Typography variant="h5" sx={{ color: Colors.charcoalGrey, fontFamily: FontFamily.NunitoRegular, mb: 4 }}>
          {state?.accountName} Account's Ledgersss
        </Typography>
