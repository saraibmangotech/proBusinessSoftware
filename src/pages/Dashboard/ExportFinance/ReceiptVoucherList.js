...
function ReceiptVoucherList() {
	...
	const getReceipts = async (page, limit, filter) => {
		try {
			...
			console.log(params);
			const { data } = await ExportFinanceServices.getReceipts(params);
													...
													{vouchers.map((item, index) => (
														...
														>
															...
															<Cell className="pdf-table" >
																...
																>
																	{
																		item?.account?.name?.length > 12
																			? item?.account?.name?.slice(0, 12) + "..."
																			: item?.account?.name
																	}
																</Tooltip>
																<Box
															...
															<Cell className="pdf-table" >$ {CommaSeparator(item?.usd_total) ?? "-"}</Cell>
															<Cell className="pdf-table" >{CommaSeparator(item?.aed_total) ?? "-"}</Cell>
															<Cell className="pdf-table" >
																<Tooltip
																	className="pdf-hide"
																	title={item?.remarks ?? "-"}
																...
																>
																	{
																		item?.remarks?.length > 24
																			? item?.remarks?.slice(0, 18) + "..."
																			: item?.remarks
																	}
																</Tooltip>
																<Box
