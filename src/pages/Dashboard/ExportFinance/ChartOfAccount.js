...
function ChartOfAccount() {
  ...
  const getChartOfAccount = async (filter) => {
    try {
      const { data } = await ExportFinanceServices.getChartOfAccount()
      ...
      {chartOfAccount ? (
        <Fragment>
          <PDFExport ref={contentRef} landscape={true} paperSize="A4" margin={5}
            fileName="All Chart of Accounts"
          >
                                            ...
                                            return (
                                              <Fragment key={j}>
                                                <Row>
                                                  ...
                                                  <Cell className="pdf-table">
                                                    {account?.childAccounts ? CommaSeparator(Balance.toFixed(2)) : CommaSeparator(Total.toFixed(2))}
                                                  </Cell>
                                                  <Cell>
                                                    {!account?.childAccounts &&
                                                      <Box component={'div'} className='pdf-hide' sx={{ gap: '16px !important' }}>
                                                        <Box onClick={() => navigate(`/account-ledger-export/${account?.id}`, { state: { accountName: account?.account_name, nature: account?.nature } })}>
                                                      ...
                                                      return (
                                                        <Fragment key={j}>
                                                          <Row sx={{ bgcolor: '#EEFBEE' }}>
                                                            ...
                                                            <Cell>
                                                              <Box component={'div'} className='pdf-hide' sx={{ gap: '16px !important' }}>
                                                                <Box onClick={() => navigate(`/account-ledger-export/${child?.id}`, { state: { accountName: child?.account_name, nature: child?.nature } })}>
                                                                  <IconButton sx={{ bgcolor: Colors.primary, '&:hover': { bgcolor: Colors.primary } }}>
                                                                    <Box component={'img'} src={Images.ledgerIcon} sx={{ height: '16px', objectFit: 'contain' }} />
                                                                  </IconButton>
                                                                  <Typography variant="body2">
