...
function BrokerCosting() {
    ...
    const updateCosting = async (formData) => {
        ...
        const allCostings = dummyData.countries.flatMap((country) =>
            country?.broker_costings?.map((costing) => ({ id: costing.id, per_unit: costing?.unit_cost, aed: parseFloat(costing?.cost_usd) * exchangeRateUsd, cost_usd: costing?.cost_usd, country_costing_id: country?.id, country_id: country?.country_id, vehicle_qty: country?.vehicle_qty, broker_id: costing?.broker_id }))
        );

        const countryTotals = allCostings.reduce((totals, item) => {
            const { country_costing_id, aed, cost_usd, vehicle_qty, country_id, broker_id } = item;
            ...
            totals[country_costing_id].total_broker_aed += parseFloat(aed);
            totals[country_costing_id].total_broker_usd += parseFloat(aed) / exchangeRateUsd;
        ...
        let obj = {
            ...
            type: 'broker',
            total_broker_aed: parseFloat(parseFloat(total_gws_aed) * parseFloat(exchangeRateUsd)).toFixed(2),
            total_broker_usd: total_gws_aed,
            brokers: allCostings,
        ...
        setDummyData((prevDummyData) => {
            const updatedCountries = prevDummyData.countries.map((country) => {
                if (country.country_id === countryId) {
                    const updatedCostings = country?.broker_costings?.map((costing) => {
                    ...
                    return { ...country, broker_costings: updatedCostings };
                }
                                            ...
                                            {dummyData?.countries?.map((item, index) => {
                                                ...
                                                return (
                                                    <Fragment key={index}>
                                                        ...
                                                        {item?.broker_costings?.map((costing, costIndex) => (
                                                            <Row key={costIndex}>
                                                                <Cell></Cell>
                                                                <Cell>{costing.exp_date ?? "-"}</Cell>
                                                                <Cell>{costing.ref_no ?? "-"}</Cell>
                                                                <Cell>{costing.broker?.name ?? "-"}</Cell>
                                ...
                                >
                                    ...
                                    >
                                        <Typography
                                            ...
                                            sx={{ color: Colors.smokeyGrey }}
                                        >
                                            {parseFloat(parseFloat(totalAed)).toFixed(2)}
                                        </Typography>
                                    </Box>
                                </Box>
                            </Grid>
