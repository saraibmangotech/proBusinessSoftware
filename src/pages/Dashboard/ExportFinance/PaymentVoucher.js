...
function PaymentVoucher() {
    ...
    const PaymentVoucher = async (formData) => {
        ...
        try {
            ...
            accountList.forEach(e => {
                let obj = {
                    ...
                    ex_loss: e?.currency == 'usd' ?  exchangeLoss : 0 ,
                }
                if (paymentMethod === 'bank') {
                    obj.bank_id = e?.id
            ...
            const { message } = await ExportFinanceServices.PaymentVoucher(obj)
            SuccessToaster(message)
            navigate('/payment-voucher-list-export')
        } catch (error) {
            ErrorToaster(error)
        } finally {
            setLoading(false)
    ...
    const updateExchangeRate = (formData) => {
        ...
        try {
            ...
            const totalExchangeLoss = (fcyUsd * formData?.rate) - (fcyUsd * usdExchangeRate)
            setExchangeLoss(totalExchangeLoss)
    ...
    return (
        <Box sx={{ m: 4, p: 5, bgcolor: Colors.white, borderRadius: 3, boxShadow: '0px 8px 18px 0px #9B9B9B1A' }}>
            ...
            <Box component="form" onSubmit={handleSubmit(PaymentVoucher)} >
