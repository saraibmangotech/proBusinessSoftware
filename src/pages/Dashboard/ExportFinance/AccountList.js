...
function AccountList() {
  ...
  const tableHead = ['COA Code', 'COA Name', 'Unit', 'Nature', 'Major Category', 'Sub Category', 'Status', 'Actions']
  ...
  const updateAccountStatus = async (id, status) => {
    ...
    try {
      ...
      const { message } = await ExportFinanceServices.updateAccount(obj)
                            ...
                            <Cell className="pdf-table">
                              <Box component={'div'} className='pdf-hide'>
                                <SwitchButton
                                  ...
                                  setIsChecked={() => permissions?.update_status && updateAccountStatus(item.id, !item?.is_active)}
                                />
