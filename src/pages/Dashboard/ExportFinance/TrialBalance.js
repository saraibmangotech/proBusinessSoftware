...
function TrialBalance() {
  ...
  const getBalanceSheet = async (filter) => {
    try {
      const { data } = await ExportFinanceServices.getAccountReports();
    ...
    filteredBalanceSheet?.forEach((item, index) => {
      ...
      item?.sub?.forEach(subItem => {
        ...
        subItem?.accounts?.forEach(account => {
          ...
          rows.push([
            ...
            account.nature == "debit" || account?.nature === null || account?.nature === "combine"
              ? parseFloat(childTotal2).toFixed(2)
              : parseFloat(0).toFixed(2), // Convert to number before calling .toFixed
            account.nature == "credit" || account?.nature === "combine"
          ...
          account?.childAccounts?.forEach(child => {
            ...
            rows.push([
              ...
              account.nature == "debit" || account?.nature === null
      ...
      item?.accounts?.map((account, j) => {
        ...
        if (account?.childAccounts?.length > 0) {
          const initialValue = { credit: 0, debit: 0, };
          const result =
            account?.childAccounts?.reduce((accumulator, transaction) => {
          ...
          Balance = account?.nature === "credit"
            ? parseFloat(result?.credit) - parseFloat(result?.debit)
            : parseFloat(result?.debit) - parseFloat(result?.credit);
        } else {
          Total = account?.nature === "credit"
            ? parseFloat(account?.total_credit) - parseFloat(account?.total_debit)
            : parseFloat(account?.total_debit) - parseFloat(account?.total_credit);
        }
        rows.push([
        ...
        account?.childAccounts?.forEach((child, j) => {
          let ChildBalance = 0;
          ChildBalance = child?.nature === "credit"
                                                      ...
                                                      <Row>
                                                        ...
                                                        >
                                                          <Typography
                                                            variant="body1"
                                                            sx={{ ml: 3 }}
                                                            className={account?.account_code + " " + 'highlighted' + " " + "pdf-table" ??
                                                              "-"}
                                                          >
                                                            <Highlighter
                                                              highlightClassName={
                                                        ...
                                                        <Cell className='pdf-table' >
                                                          <Highlighter
                                                            highlightClassName={
                                                              account?.account_category + " " + "pdf-table" ??
                                                              "-"
                                                            }
                                                            searchWords={[
                                                              textValue,
                                                                ...
                                                                return (
                                                                  <Fragment key={j}>
                                                                    ...
                                                                    >
                                                                      ...
                                                                      <Cell>
                                                                        <Box component={'div'} className='pdf-table'
                                                                          sx={{
                                                                            gap: "16px !important",
                                                                          }}
                                                                        >
