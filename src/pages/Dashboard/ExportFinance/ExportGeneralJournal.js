...
function GeneralJournalLedger() {
	...
	const getGeneralJournalLedgers = async (page, limit, filter) => {
		...
		try {
			...
			const { data } = await ExportFinanceServices.getGeneralJournalLedgers(params)
	...
	const renderCellContent = (colIndex, item, isActive,) => {
		...
		switch (colIndex) {
			...
			case 2:
				return (
					<Box>
						...
						>
							{item.entry?.reference_no?.length > 24 ? item.entry?.reference_no?.slice(0, 8) + "..." : item.entry?.reference_no}
						</Tooltip>
						<Box
							component={"div"}
							className="pdf-show"
			...
			case 6:
				return CommaSeparator(parseFloat(item?.debit).toFixed(2)) ??
					"0.00";
			case 7:
				return CommaSeparator(parseFloat(item?.credit).toFixed(2)) ?? "0.00";
			case 8:
			...
			case 9:
				return (
					<Box>
						...
						>{item?.comment?.length > 24 ? item?.comment?.slice(0, 8) + "..." : item?.comment}
						</Tooltip>
						<Box
							component={"div"}
							className="pdf-show"
	...
	const downloadExcel = () => {
		...
		const rows = generalJournalAccounts?.map((item) => [
						...
						generalJournalAccounts && (
							<Fragment>
								<PDFExport ref={contentRef} landscape={true} paperSize="A4" margin={5}
									fileName="General Journal Ledger"
								>
