...
function Permission() {
  ...
  function filterPermitted(items) {
    return items
      .map((item) => {
        // Recursively filter children if they exist
        const filteredChildren = item.children ? filterPermitted(item.children) : [];
  
        // Include the item if it is permitted or has permitted children
        if (item.permitted || filteredChildren.length > 0) {
          return {
            ...item,
            children: filteredChildren,
          };
        }
        return null; // Exclude items that are neither permitted nor have permitted children
      })
  ...
  const getRolesPermissions = async (permittedItems) => {
    setLoader(true)
    try {
      let params = {
        role_id: id
      }
      const { data } = await RoleServices.getSubRolesPermissions(params)
  ...
  const saveRolesPermissions = async () => {
    ...
    try {
      ...
      const { message } = await RoleServices.updateRolesPermissions(obj)
  ...
  return (
    <Box sx={{ m: 4, mb: 2 }}>
      ...
      <Box onClick={() => navigate('/sub-role-list')} sx={{ width: '100px', mb: 2, cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
        <IconButton
          size="small"
          sx={{
