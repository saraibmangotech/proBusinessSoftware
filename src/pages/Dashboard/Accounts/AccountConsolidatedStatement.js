...
function AccountConsolidatedProStatement() {
    ...
    const prepareXLSXData = (data) => {
        ...
        worksheet.mergeCells("A2:I2"); // Adjusted from G2 to I2
        ...
        worksheet.addRow([]);
        // Add table headers
        const headers = ["Type", "Comment", "Description", "#", "Date", "Due Date", "Debit", "Credit", "Balance"]; // Updated headers
        ...
        data.forEach((row) => {
            ...
            if (row.isAccountHeader) {
                dataRow = worksheet.addRow([row.account.name, "", "", "", "", "", "", "", ""]); // Adjusted colSpan for 9 columns
                ...
                worksheet.mergeCells(`A${dataRow.number}:I${dataRow.number}`); // Merge across 9 columns
                return;
            }
            // Opening Balance Row
            if (row.isOpeningBalance) {
                dataRow = worksheet.addRow([
                    ...
                    "", // Due Date
                    Number.parseFloat(row.debit || 0), // Debit
                    Number.parseFloat(row.credit || 0), // Credit
                    Number.parseFloat(row.runningBalance || 0), // Balance
                ]);
                dataRow.getCell(5).font = { name: "Arial", size: 10, bold: true }; // Adjusted cell number for "Opening Balance"
                // Format numeric columns
                [7, 8, 9].forEach(colNum => { // Adjusted col numbers (Debit, Credit, Balance)
            ...
            if (row.isTotal) {
                dataRow = worksheet.addRow([
                    "Total",
                    "", "", "", "", "", // Empty cells for Document, Description, #, Date, Due Date
                    Number.parseFloat(row.debit || 0), // Debit
                    Number.parseFloat(row.credit || 0), // Credit
                    Number.parseFloat(row.runningBalance || 0), // Balance
                ]);
                worksheet.mergeCells(`A${dataRow.number}:F${dataRow.number}`); // Merge "Total" across 6 columns
                dataRow.eachCell((cell, colNumber) => {
                    if (colNumber === 1) {
                        cell.font = { name: "Arial", size: 10, bold: true };
                    }
                    if ([7, 8, 9].includes(colNumber)) { // Adjusted col numbers (Debit, Credit, Balance)
                        cell.fill = {
                            type: "pattern",
                            pattern: "solid",
            ...
            const comment = row?.comment || (row?.entry?.comment || '-'); // Assuming comment exists on row or entry
            const description = row?.description || (row?.entry?.description || '-'); // Assuming description exists on row or entry
        ...
        if (data.length > 0) {
            ...
            worksheet.mergeCells(`A${grandTotalRow.number}:F${grandTotalRow.number}`); // Merge "Grand Total" across 6 columns
        ...
        worksheet.mergeCells(`A${reportRow.number}:I${reportRow.number}`); // Adjusted from G to I
        // Set column widths
        worksheet.columns = [
            { width: 15 }, // Type
            { width: 20 }, // Document (New)
            { width: 30 }, // Description (New)
            { width: 15 }, // #
            { width: 12 }, // Date
            { width: 12 }, // Due Date
            { width: 15 }, // Debit
        ...
        worksheet.mergeCells(`A${system2.number}:I${system2.number}`); // Adjusted from G to I
    ...
    const headers = [
        { label: "Type", key: "Type" },
        { label: "Document", key: "Document" }, // New
        { label: "Description", key: "Description" }, // New
        { label: "#", key: "#" },
        { label: "Date", key: "Date" },
    ...
    const computeRunningBalance = (allAccountsData, setClosingBal) => {
        ...
        allAccountsData.forEach((accountData) => {
            if (accountData.account && accountData.account.name) {
                const isNegate = accountData.account.primary_account_id === 700328;
                // Add account header
                processedData.push({
                    isAccountHeader: true,
                    id: `header-${accountData.account.id}`,
                    account: accountData.account,
                    comment: '', // Added for consistency
                    description: '', // Added for consistency
                });
                // Add opening balance row if available
                if (accountData.opening_balance !== null && accountData.opening_balance !== undefined) {
                    const openingBalance = Number.parseFloat(accountData.opening_balance || 0.0);
                    processedData.push({
                        type: { type_name: "" },
                        id: `opening-${accountData.account.id}`,
                        isOpeningBalance: true,
                        debit: 0.0,
                        credit: 0.0,
                        allocated: 0.0,
                        runningBalance: isNegate ? -1 * openingBalance : openingBalance,
                        account: accountData.account, // required later in Excel for balance negation
                        comment: '', // Added for consistency
                        description: '', // Added for consistency
                    });
                }
                ...
                let totalDebit = 0;
                let totalCredit = 0;
                let runningBalance = Number.parseFloat(accountData.opening_balance || 0);
                if (accountData.statement && accountData.statement.length > 0) {
                    accountData.statement.forEach((entry) => {
                        ...
                        processedData.push({
                            ...
                            account: accountData.account, // required in Excel
                            comment: entry?.comment || '', // Assuming entry has comment
                            description: entry?.description || '', // Assuming entry has description
                        });
                    });
                }
                // Add total row
    ...
    const columns = [
        ...
        {
            header: "Document", // New column definition
            accessorKey: "comment",
            cell: ({ row }) => (
                <Box variant="contained" color="primary" sx={{ cursor: "pointer", display: "flex", gap: 2 }}>
                    {row?.original?.comment || (row?.original?.entry?.comment || '-')}
                </Box>
            ),
        },
        {
            header: "Description", // New column definition
            accessorKey: "description",
            cell: ({ row }) => (
                <Box variant="contained" color="primary" sx={{ cursor: "pointer", display: "flex", gap: 2 }}>
                    {row?.original?.description || (row?.original?.entry?.description || '-')}
                </Box>
            ),
        },
        {
                        ...
                        <tbody>
                                ...
                                customerQueue.map((row, index) => {
                                    ...
                                    if (row.isAccountHeader) {
                                        return (
                                            <tr key={row.id} className="account-header">
                                                <td colSpan={tableHead.length} className="account-name">
                                                    {row.account.name}
                                                </td>
                                            </tr>
                                        )
                                    ...
                                    if (row.isOpeningBalance) {
                                        return (
                                            <tr key={row.id} className="opening-balance-row">
                                                <td></td> {/* Type */}
                                                <td></td> {/* Document (new) */}
                                                <td></td> {/* Description (new) */}
                                                <td></td> {/* # */}
                                                <td>Opening Balance</td> {/* Date */}
                                                <td></td> {/* Due Date */}
                                                <td className="number-cell">{Number.parseFloat(row.debit || 0).toFixed(2)}</td> {/* Debit */}
                                                <td className="number-cell">{Number.parseFloat(row.credit || 0).toFixed(2)}</td> {/* Credit */}
                                                <td
                                    ...
                                    if (row.isTotal) {
                                        return (
                                            <tr key={row.id} className="total-row">
                                                <td colSpan={6} className="bold">Total</td> {/* Adjusted colSpan */}
                                    ...
                                    return (
                                        <tr key={row.id || index} className="transaction-row">
                                            ...
                                            <td>{row?.comment || (row?.entry?.comment || '-')}</td> {/* New */}
                                            <td>{row?.description || (row?.entry?.description || '-')}</td> {/* New */}
                                            <td>
                            ...
                            {!loader && customerQueue?.length > 0 && (
                                <tr className="grand-total-row">
                                    <td colSpan={6} className="bold"> {/* Adjusted colSpan */}
                                        Grand Total
                                    </td>
