...
function ConsolidatedCustomerReceivables() {
  ...
  const calculateGrandTotals = (data) => {
    ...
    data.forEach((row) => {
      if (!row.isCustomerHeader && !row.isTotal && !row.isOpeningBalance) {
  ...
  const prepareExcelData = (data) => {
    ...
    const worksheet = workbook.addWorksheet("Consolidated Customer Receivables")

    worksheet.headerFooter.oddHeader =
      '&C&"Arial,Bold"&18CONSOLIDATED CUSTOMER RECEIVABLES\n' +
    ...
    worksheet.addRow(["CONSOLIDATED CUSTOMER RECEIVABLES"]).getCell(1).font = {
      name: "Arial",
      size: 16,
      bold: true,
      color: { argb: "2F4F4F" },
    ...
    worksheet.addRow(["Customer:", selectedUser ? "Selected Customers" : "All"])
    worksheet.addRow(["Currency:", "AED"])
    worksheet.addRow([])

    if (selectedUser && selectedUser.length > 0) {
      worksheet.addRow(["Selected Customers:"]).getCell(1).font = {
        name: "Arial",
        size: 11,
        bold: true,
        color: { argb: "2F4F4F" },
      }
      selectedUser.forEach((customer) => worksheet.addRow([`   • ${customer.name || customer}`]))
      worksheet.addRow([])
    }
    ...
    data.forEach((row) => {
      ...
      if (row.isCustomerHeader) {
        dataRow = worksheet.addRow([row.customer_name])
      ...
      const date = row.invoice_date ? moment(row.invoice_date).format("YYYY-MM-DD") : ""
      const dueDate = row.invoice_date ? moment(row.invoice_date).format("YYYY-MM-DD") : ""
    ...
    workbook.properties = {
      title: "Consolidated Customer Receivables",
      subject: "Financial Report",
      keywords: "customer receivables, financial, accounting",
      category: "Financial Reports",
      description: "Customer consolidated receivables generated from accounting system",
    ...
    const download = async () => {
      ...
      saveAs(blob, "Consolidated_Customer_Receivables.xlsx")
    }
  ...
  const columns = [
      ...
      cell: ({ row }) => (
        <Box
          ...
          sx={{
            ...
            display: "flex",
            gap: 2,
            fontWeight: row?.original?.isCustomerHeader ? "bold" : "normal",
            backgroundColor: row?.original?.isCustomerHeader ? "#f5f5f5" : "transparent",
            color: row?.original?.isTotal ? "#1976d2" : "inherit",
          }}
        >
          {row?.original?.isCustomerHeader ? row?.original?.customer_name : row?.original?.type || "Invoice"}
        </Box>
      ),
    },
    {
    ...
    },
    {
      id: "invoice_date",
      header: "Date",
      accessorFn: (row) => (row.invoice_date ? moment(row.invoice_date).format("YYYY-MM-DD") : ""),
      cell: ({ row }) => (
        <Box variant="contained" color="primary" sx={{ cursor: "pointer", display: "flex", gap: 2 }}>
          {row.original.invoice_date ? moment(row.original.invoice_date).format("YYYY-MM-DD") : ""}
        </Box>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Box variant="contained" color="primary" sx={{ cursor: "pointer", display: "flex", gap: 2 }}>
          {Number.parseFloat(row?.original?.total_amount || 0).toFixed(2)}
        </Box>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <Box variant="contained" color="primary" sx={{ cursor: "pointer", display: "flex", gap: 2 }}>
          {Number.parseFloat(row?.original?.paid_amount || 0).toFixed(2)}
        </Box>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <Box variant="contained" color="primary" sx={{ cursor: "pointer", display: "flex", gap: 2 }}>
          {Number.parseFloat(row?.original?.unpaid_amount || 0).toFixed(2)}
        </Box>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        ...
        >
          <span className={`status-badge ${row?.original?.status === "Paid" ? "status-paid" : "status-unpaid"}`}>
    ...
    if (true) {
      ...
      try {
        const params = {
          ...
          customers: selectedUser.map((user) => user.id).join(","),
          account_id: 700111,
        }
          ...
          customers: [
            {
              ...
              invoices: [
                {
                  invoice_id: 65015252,
                  invoice_number: "DED/20012119",
                  invoice_date: "2025-07-02T00:00:00.000Z",
                  total_amount: 111.75,
                  total_vat: 3.75,
                  unpaid_amount: 115.5,
                },
                {
                  invoice_id: 65015251,
                  invoice_number: "DED/20012118",
                  invoice_date: "2025-07-02T00:00:00.000Z",
                  total_amount: 127,
                  total_vat: 4.25,
                  unpaid_amount: 131.25,
                },
              ],
              total_unpaid: 246.75,
            },
          ],
          overall_unpaid: 246.75,
        }

        const { data } = await FinanceServices.getConsolidatedCustomerReceivable(params)

        // Process the data to create the table rows
        const processedData = processStatementsData(data?.customers || [])
        console.log(processedData, "processedData")

        setCustomerQueue(processedData)
        setTotalCount(data?.customers?.reduce((total, vendor) => total + (vendor.invoices?.length || 0), 0))
  ...
  const processStatementsData = (customers) => {
    const result = []
    ...
    customers.forEach((customer) => {
      if (customer.customer_name) {
        ...
        result.push({
          ...
          id: `header-${customer.customer_id}`,
        ...
        result.push({
          type: "",
          id: `opening-${customer.customer_id}`,
        ...
        if (customer.invoices && customer.invoices.length > 0) {
          customer.invoices.forEach((invoice) => {
            ...
            const paid = amount - unpaid // Calculate paid amount
        ...
        // Add total row for this customer with new fields:
        result.push({
          id: `total-${customer.customer_id}`,
          isTotal: true,
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
        <Typography sx={{ fontSize: "24px", fontWeight: "bold" }}>Consolidated Customer Receivables</Typography>
      </Box>
                ...
                customerQueue.map((row, index) => {
                  ...
                  if (row.isCustomerHeader) {
                    return (
                      <tr key={row.id} className="account-header">
                        <td colSpan={8} className="account-name">
                          {row.customer_name}
                        </td>
                      </tr>
                    )
                  ...
                  return (
                    <tr key={row.id || index} className="transaction-row">
                      ...
                      <td>{row.invoice_date ? moment(row.invoice_date).format("YYYY-MM-DD") : ""}</td>
                      <td>{row.invoice_date ? moment(row.invoice_date).format("YYYY-MM-DD") : ""}</td>
...
export default ConsolidatedCustomerReceivables
