...
function GeneralLedger() {
  ...
  const [accountLedgers3, setAccountLedgers3] = useState([])
  const getAccountLedgers3 = async (id,name) => {
    setLoading(true);
    try {
      ...
      downloadExcel2(data?.statement?.rows,name)
  ...
  const downloadExcel2 = (newData,name) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Account Ledger");
      ...
      {/* <PrimaryButton
            ...
            onClick={async() => {
              ...
              for (let index = 0; index < accounts.length; index++) {
               await getAccountLedgers3(accounts[index]?.id,accounts[index]?.name)
