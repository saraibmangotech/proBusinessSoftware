...
import { Box, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography, tableCellClasses, IconButton, CircularProgress, Chip, Grid, InputAdornment } from '@mui/material';
...
function AccountApprovalList() {
  ...
  const getAccountsApprovals = async (page, limit, filter) => {
    ...
    try {
      ...
      const { data } = await FinanceServices.getAccountsApprovals(params)
  ...
  const handleStatusAction = (data) => {
    if (permissions?.approve && data?.is_approved === false) {
  ...
  const approveAccount = async () => {
    try {
      ...
      const { message } = await FinanceServices.approveAccount(obj)
