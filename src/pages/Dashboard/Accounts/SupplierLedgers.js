  ...
  const prepareCSVDataXLSX = (data) => {
    ...
    data.forEach((item) => {
      ...
      const dataRow = worksheet.addRow([
        ...
        item?.cost_center || "",
        item.account?.name || "",
        debit, // Use number directly, not .toFixed(2)
        credit, // Use number directly, not .toFixed(2)
        runningBalance, // Use number directly, not .toFixed(2)
      ]);
    ...
    const totalRow = worksheet.addRow([
      ...
      "",
      "Total",
      totalDebit, // Use number directly, not .toFixed(2)
      totalCredit, // Use number directly, not .toFixed(2)
      runningBalance, // Final closing balance as number, not .toFixed(2)
    ]);
    ...
    workbook.properties = {
      ...
      description: "Ledger report generated from accounting system",
      company: name,
    };
    ...
    const download = async () => {
      ...
      saveAs(blob, 'Supplier_Ledgers.xlsx');
    };
    download();
  };
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
          {customerQueue?.length > 0 &&
            <Button
              ...
              onClick={() => prepareCSVDataXLSX(tableData)}
              variant="contained"
              color="primary"
              sx={{
                padding: '10px',
