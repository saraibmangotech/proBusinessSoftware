...
function AccountPayrollList() {
    ...
    const getCustomerQueue = async (date) => {
        ...
        try {
            ...
            const { data } = await CustomerServices.getSalaries(params)
            setCustomerQueue(data?.salaries?.rows)
    ...
    const UpdateStatus = async () => {
        try {
            ...
            const promise = CustomerServices.updateSalaryStatus(obj);
            console.log(promise);
...

        {
            ...
            cell: ({ row }) => (
                <Box sx={{ display: 'flex', gap: 1 }}>
                    ...
                    {( user?.employee_id== 1202  ) && (
                        <Tooltip title="Detail">
                            <Box
                                component="img"
                                sx={{ cursor: "pointer" }}
                                onClick={() => {
                                   navigate(`/salary-detail/${row?.original?.id}`);
                                }}
                                src={Images.detailIcon}
                                width="35px"
                            />
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography sx={{ fontSize: '24px', fontWeight: 'bold' }}>Account Payroll List</Typography>
...
export default AccountPayrollList;