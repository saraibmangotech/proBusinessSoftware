...
function MobayaReversals() {

  const navigate = useNavigate();
  const classes = useStyles();
  const contentRef = useRef(null);

  const tableHead = ['Customer ID', 'Customer Name', 'VIN', 'LOT', 'Make', 'Modal', 'Color', 'Purpose', 'Reversed At', 'Resversed By', 'Mobaya Date', 'Mobaya Reference']
  ...
  const getMobaya = async (page, limit, filter) => {
    ...
    try {
      ...
      const { data } = await VccServices.getMobayaReversals(params)
  ...
  const renderCellContent = (colIndex, item, isActive,) => {
    ...
    switch (colIndex) {
      ...
      case 9:
        return (
          <Box>
            <Tooltip
              className='pdf-hide'
              title={item?.creator ? item?.creator?.name : '-'}
              arrow
              placement="top"
              slotProps={{
                popper: {
      ...
      // item?.creator ? item?.creator?.name : '-';
      case 10:
        return item?.mobaya_date ? moment(item?.mobaya_date).format('MM-DD-YYYY') : '-';
      case 11:
        return item?.mobaya_reference ?? '-'
      default:
        return "-";
    }
  };
  ...
  const downloadExcel = () => {
    ...
    const rows = vehicleList?.map((item) => {
      ...
      return [
        ...
        item?.booking?.veh_model?.name ?? '-',
        item?.booking?.color ?? '-',
        item?.changed_purpose ?? '-',
        item?.created_at ? moment(item?.created_at).format('MM-DD-YYYY') : '-',
        item?.creator ? item?.creator?.name : '-',
        item?.mobaya_date ? moment(item?.mobaya_date).format('MM-DD-YYYY') : '-',
  ...
  return (
    <Box sx={{ m: 4, mb: 2 }}>
      ...
      {/* ========== Receive Dialog ========== */}
      <GatePassReceiveDialog open={receiveStatusDialog} onClose={() => setReceiveStatusDialog(false)} updateStatus={() => receiveMakasa()} page={'MobayaReversals'} />
              ...
              vehicleList && (
                <Fragment>
                  <PDFExport ref={contentRef} landscape={true} paperSize="A4" margin={5}
                    fileName="Vehicle Sales Agreement Reversals"
                  >
...
export default MobayaReversals;