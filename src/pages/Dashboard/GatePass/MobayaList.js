...
function MobayaList() {
  ...
  const tableHead = ['Customer ID', 'Customer Name', 'VIN', 'LOT', 'Make', 'Modal', 'Color', 'VCC Issue Purpose', 'VCC Declaration Number', 'VCC Declaration Date', 'VCC Expiry Date', 'Time Left', 'Status',]
  ...
  const getMobaya = async (page, limit, filter) => {
    setLoader(true)
    try {
      ...
      const { data } = await VccServices.getMobaya(params)
  ...
  const renderCellContent = (colIndex, item, isActive,) => {
    ...
    switch (colIndex) {
      ...
      case 12:
        return <Box sx={{ cursor: 'pointer', 'path': { fill: item?.vehicle?.mobaya_issued_date ? Colors.primary : item?.vehicle?.mobaya_issued_date === null ? Colors.danger : '' } }}>
          <span className='pdf-hide'> {item?.vehicle?.mobaya_issued_date ? <CheckIcon /> : <PendingIcon />} </span>
          <Typography variant="body2">
            {item?.vehicle?.mobaya_issued_date ? 'Issued' : 'Not Issued'}
          </Typography>
        </Box>;
      default:
        return "-";
  ...
  return (
    <Box sx={{ m: 4, mb: 2 }}>
      ...
      {/* ========== Receive Dialog ========== */}
      <GatePassReceiveDialog open={receiveStatusDialog} onClose={() => setReceiveStatusDialog(false)} updateStatus={() => receiveMakasa()} page={'MobayaList'} />
              ...
              vehicleList && (
                <Fragment>
                  <PDFExport ref={contentRef} landscape={true} paperSize="A4" margin={5}
                    fileName="Vehicle Sales Agreement List"
                  >
                  ...
                  <Pagination
                    ...
                    pageSize={pageLimit}
                    onPageSizeChange={(size) => getMobaya(1, size.target.value)}
                    tableCount={vehicleList?.length}
                    totalCount={totalCount}
                    onPageChange={(page) => getMobaya(page, "")}
                  />
